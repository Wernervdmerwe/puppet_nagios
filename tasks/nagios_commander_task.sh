...
unalias -a
# globals can be defined here if desired #NAG_HOST=''
#USERNAME=''
#PASSWORD=''
...
function ACKNOWLEDGE {
	curl -sS  $DATA $NAGIOS_INSTANCE/cmd.cgi --data host=$HOST --data "com_data=$COMMENT" --data cmd_mod=2 --data btnSubmit=Commit -u $USERNAME:$PASSWORD | grep -o 'Your command request was successfully submitted to Nagios for processing.'
	if [ $? -eq 1 ]; then 
